// src/lib/supabase/client.ts - VERSI�N CORRECTA
"use client";
...
export const useSupabase = () => {
  ...
  if (typeof window === "undefined") {
    return { 
      client: null, 
      isLoading: false, 
      error: "Llamada en servidor. Las variables deber�an estar disponibles." 
    };
  }
  
  // DEBUG: Log para ver qu� hay disponible
  ...
  if (missingVars.length > 0) {
    ...
    Esto significa que las variables de entorno NO est�n configuradas en Vercel o no se est�n inyectando.
    ...
    // Tambi�n mostrar alerta en producci�n para el usuario
    if (typeof window !== 'undefined' && process.env.NODE_ENV === 'production') {
      setTimeout(() => {
        if (!localStorage.getItem('env_error_shown')) {
          alert('?? Error de configuraci�n: Variables de entorno faltantes. Por favor, contacta al administrador.');
