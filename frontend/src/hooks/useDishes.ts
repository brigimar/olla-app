...
const fetchPopularDishes = async ({ limit = 10, city }: { limit?: number; city?: string }) => {
  try {
    if (city) {
...
export const usePopularDishes = (options: UsePopularDishesOptions = {}) => {
  const { limit = 10, city, enabled = true, staleTime = 1000 * 60 * 5 } = options;
...
export const useDishById = (id: string, options: UseDishesOptions = {}) => {
...
export const useDishesByCity = (city: string, options: UseDishesOptions = {}) => {
  const { enabled = true, staleTime = 1000 * 60 * 5 } = options;
