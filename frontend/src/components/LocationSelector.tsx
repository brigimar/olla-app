...
const LocationSelector = ({
  ...
  // --- Detect partido desde coordenadas ---
  const isPointInPolygon = useCallback(
    ([lat, lng]: [number, number], polygon: [number, number][]) => {
      let inside = false;
  ...
  const findPartidoFromCoords = useCallback(
    (lat: number, lng: number): PartidoAMBA | null => {
      for (const partido of PARTIDOS_AMBA) {
        if (isPointInPolygon([lat, lng], partido.poligono)) return partido;
      }
      return null;
    },
    [isPointInPolygon]
  ...
  const useCurrentLocation = () => getCurrentLocation();
