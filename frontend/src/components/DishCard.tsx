...
const FALLBACK_IMAGE = '/images/placeholder-dish.jpg'; // asegúrate de tener esta imagen en public/images

export default function DishCard({ dish, onAddToCart, className }: DishCardProps) {
  // ─── Formateo de precio ─────────────────────────────────────────────────────
  ...
  // ─── Manejo de imagen ───────────────────────────────────────────────────────
  const imageUrl = dish.image_url && dish.image_url.trim() !== '' ? dish.image_url : FALLBACK_IMAGE;
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="flex justify-between">
          ...
          {dish.rating > 0 ? (
            <span className="text-xs font-medium text-gray-500">{dish.rating.toFixed(1)}</span>
          ) : null}
        </div>
        ...
        <div className="mt-2">
          {dish.rating > 0 ? (
            ...
            !badge && <span className="text-xs text-gray-400">Sin valoraciones</span>
          )}
        </div>
